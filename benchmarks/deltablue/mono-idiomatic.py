...
class Strength:
  ...
  def nextWeaker(self):
    if self.strengthValue == 0:
      return Strength.Weakest()
    if self.strengthValue == 1:
      return Strength.WeakDefault()
    if self.strengthValue == 2:
      return Strength.Normal()
    if self.strengthValue == 3:
      return Strength.StrongDefault()
    if self.strengthValue == 4:
      return Strength.Preferred()
    if self.strengthValue == 5:
      return Strength.Required()
...
class BinaryConstraint(Constraint):
  ...
  def chooseMethod(self, mark):
    if self.v1.mark == mark:
      self.direction = Direction.FORWARD if (self.v2.mark != mark and Strength.stronger(self.strength, self.v2.walkStrength)) else Direction.NONE

    if self.v2.mark == mark:
      self.direction = Direction.BACKWARD if (self.v1.mark != mark and Strength.stronger(self.strength, self.v1.walkStrength)) else Direction.NONE

    if Strength.weaker(self.v1.walkStrength, self.v2.walkStrength):
      self.direction = Direction.BACKWARD if Strength.stronger(self.strength, self.v1.walkStrength) else Direction.NONE
    else:
      self.direction = Direction.FORWARD if Strength.stronger(self.strength, self.v2.walkStrength) else Direction.BACKWARD
...
class EqualityConstraint(BinaryConstraint):
  ...
  def __init__(self, var1, var2, strength):
    BinaryConstraint.__init__(self, var1, var2, strength)
...
class Planner:
  ...
  def incrementalRemove(self, c):
    ...
    while True:
      for i in xrange(0, unsatisfied.size()):
        u = unsatisfied.at(i)
  ...
  def removePropagateFrom(self, out):
    ...
    while todo.size() > 0:
      v = todo.removeFirst()
      for i in xrange(0, v.constraints.size()):
