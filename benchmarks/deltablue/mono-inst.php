...
// STARTFUNC
function alert($s) { echo "CALL alert\n";
...
class OrderedCollection {
  ...
  // STARTFUNC
  function add($elm) { echo "CALL add\n";
    array_push($this->elms, $elm);
  }
  // ENDFUNC

  // STARTFUNC
  function at($index) { echo "CALL at\n";
    return $this->elms[$index];
  }
  // ENDFUNC

  // STARTFUNC
  function size() { echo "CALL size\n";
    return count($this->elms);
  }
  // ENDFUNC

  // STARTFUNC
  function removeFirst() { echo "CALL removeFirst\n";
    return array_pop($this->elms);
  }
  // ENDFUNC

  // STARTFUNC
  function remove($elm) { echo "CALL remove\n";
    $index = 0;
    $skipped = 0;
...
class Strength {
  ...
  // STARTFUNC
  function __construct($strengthValue, $name) { echo "CALL __construct\n";
  ...
  // STARTFUNC
  public static function stronger($s1, $s2) { echo "CALL stronger\n";
    return $s1->strengthValue < $s2->strengthValue;
  }
  // ENDFUNC

  // STARTFUNC
  public static function weaker($s1, $s2) { echo "CALL weaker\n";
    return $s1->strengthValue > $s2->strengthValue;
  }
  // ENDFUNC

  // STARTFUNC
  public static function weakestOf($s1, $s2) { echo "CALL weakestOf\n";
    return Strength::weaker($s1, $s2) ? $s1 : $s2;
  }
  // ENDFUNC

  // STARTFUNC
  public static function strongest($s1, $s2) { echo "CALL strongest\n";
    return Strength::stronger($s1, $s2) ? $s1 : $s2;
  }
  // ENDFUNC

  // STARTFUNC
  function nextWeaker() { echo "CALL nextWeaker\n";
...
class Constraint {
  ...
  // STARTFUNC
  function __construct($strength) { echo "CALL __construct\n";
  ...
  // STARTFUNC
  function addConstraint() { echo "CALL addConstraint\n";
    global $planner;
  ...
  // STARTFUNC
  function satisfy($mark) { echo "CALL satisfy\n";
    global $planner;
  ...
  // STARTFUNC
  function destroyConstraint() { echo "CALL destroyConstraint\n";
    global $planner;
  ...
  // STARTFUNC
  function isInput() { echo "CALL isInput\n";
    return false;
  }
  // ENDFUNC
}
...
class UnaryConstraint extends Constraint {
  ...
  // STARTFUNC
  function __construct($v, $strength) { echo "CALL __construct\n";
  ...
  // STARTFUNC
  function addToGraph() { echo "CALL addToGraph\n";
  ...
  // STARTFUNC
  function chooseMethod($mark) { echo "CALL chooseMethod\n";
  ...
  // STARTFUNC
  function isSatisfied() { echo "CALL isSatisfied\n";
    return $this->satisfied;
  }
  // ENDFUNC

  // STARTFUNC
  function markInputs($mark) { echo "CALL markInputs\n";
    // has no inputs
  }
  // ENDFUNC
  ...
  // STARTFUNC
  function output() { echo "CALL output\n";
  ...
  // STARTFUNC
  function recalculate() { echo "CALL recalculate\n";
  ...
  // STARTFUNC
  function markUnsatisfied() { echo "CALL markUnsatisfied\n";
    $this->satisfied = false;
  }
  // ENDFUNC

  // STARTFUNC
  function inputsKnown() { echo "CALL inputsKnown\n";
    return true;
  }
  // ENDFUNC

  // STARTFUNC
  function removeFromGraph() { echo "CALL removeFromGraph\n";
...
class StayConstraint extends UnaryConstraint {
  // STARTFUNC
  function __construct($v, $str) { echo "CALL __construct\n";
    parent::__construct($v, $str);
  }
  // ENDFUNC

  // STARTFUNC
  function execute() { echo "CALL execute\n";
...
class BinaryConstraint extends Constraint {
  ...
  // STARTFUNC
  function __construct($var1, $var2, $strength) { echo "CALL __construct\n";
    parent::__construct($strength);
    $this->v1 = $var1;
    $this->v2 = $var2;
  ...
  // STARTFUNC
  function input() { echo "CALL input\n";
  ...
  // STARTFUNC
  function inputsKnown($mark) { echo "CALL inputsKnown\n";
    $i = $this->input();
...
class ScaleConstraint extends BinaryConstraint {
  ...
  // STARTFUNC
  function __construct($src, $scale, $offset, $dest, $strength) { echo "CALL __construct\n";
    $this->direction = Direction::NONE;
    $this->scale = $scale;
    $this->offset = $offset;
...
class Variable {
  ...
  // STARTFUNC
  function __construct($name, $initialValue = null) { echo "CALL __construct\n";
  ...
  // STARTFUNC
  function addConstraint($c) { echo "CALL addConstraint\n";
    $this->constraints->add($c);
  }
  // ENDFUNC

  // STARTFUNC
  function removeConstraint($c) { echo "CALL removeConstraint\n";
...
class Planner {
  ...
  // STARTFUNC
  function incrementalAdd($c) { echo "CALL incrementalAdd\n";
    $mark = $this->newMark();
  ...
  // STARTFUNC
  function incrementalRemove($c) { echo "CALL incrementalRemove\n";
    $out = $c->output();
    $c->markUnsatisfied();
    $c->removeFromGraph();
  ...
  // STARTFUNC
  function newMark() { echo "CALL newMark\n";
    return ++$this->currentMark;
  }
  // ENDFUNC

  // STARTFUNC
  function makePlan($sources) { echo "CALL makePlan\n";
    $mark = $this->newMark();
    $plan = new Plan();
    $todo = $sources;
    while ($todo->size() > 0) {
  ...
  // STARTFUNC
  function extractPlanFromConstraints($constraints) { echo "CALL extractPlanFromConstraints\n";
  ...
  // STARTFUNC
  function addPropagate($c, $mark) { echo "CALL addPropagate\n";
  ...
  // STARTFUNC
  function removePropagateFrom($out) { echo "CALL removePropagateFrom\n";
  ...
  // STARTFUNC
  function addConstraintsConsumingTo($v, $coll) { echo "CALL addConstraintsConsumingTo\n";
    $determining = $v->determinedBy;
    $cc = $v->constraints;
    for ($i = 0; $i < $cc->size(); $i++) {
      $c = $cc->at($i);
...
class Plan {
  ...
  // STARTFUNC
  function constraintAt($index) { echo "CALL constraintAt\n";
...
// ENDCLASS 13
  ...
  // STARTFUNC
function chainTest($n) { echo "CALL chainTest\n";
  global $planner;
  ...
  // STARTFUNC
function projectionTest($n) { echo "CALL projectionTest\n";
  global $planner;
  ...
  // STARTFUNC
function change($v, $newValue) { echo "CALL change\n";
  global $planner;
